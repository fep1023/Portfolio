function RandomNumber(seed,type) {
    var new_seed = Math.floor(5*Math.sin(2-6*seed)-4*Math.cos(1+2*seed));
    var output = "";
    
    if (type==1) {
        return GetUC(12*Math.abs(Math.floor(3*Math.sin(1-2*new_seed)-4*Math.cos(10+2*new_seed))));
    } else if (type==2) {
        return GetLC(43*Math.abs(Math.floor(7*Math.sin(2-4*seed)-8*Math.cos(11+4*new_seed))));
    } else {
        return GetNum(35*Math.abs(Math.floor(11*Math.sin(3-8*new_seed)-16*Math.cos(12+6*new_seed))));
    }
}
...
var bttn = document.getElementById("function");
